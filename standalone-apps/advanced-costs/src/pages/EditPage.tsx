...
import { getMockCostById, updateMockCost } from '../mockAdvancedCosts';
import {
...
export default function EditPage() {
  ...
  const record = getMockCostById(id!);

  const methods = useForm<FormValues>({
    defaultValues: record
      ? { name: record.name, costType: record.costType, affiliation: record.affiliation, hotels: (record as any).hotels ?? [],
  ...
  if (!record) { navigate('/'); return null; }

  const onSubmit = (v: FormValues) => {
    updateMockCost(id!, { ...v, schedule: v.schedule as 'continuous' | 'scheduled', active: record.active });
    setSaved(true);
    setTimeout(() => navigate('/'), 1500);
  };

  return (
    <Modal title={record.name} onClose={() => navigate('/')}>
          ...
          <div className="form-row">
            <div className="field-group">
              ...
              <Controller name="name" control={control}
                render={({ field }) => <input className="form-input" style={{ width: 240 }} {...field} />} />
            </div>
      ...
      {saved && <div className="snackbar">✓ Cost updated successfully</div>}
    </Modal>
  );
}
